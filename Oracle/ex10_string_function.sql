...
/*
    substr()
    - 문자열 추출함수
    - varchar2 substr(컬럼, 시작위치, 가져올 문자개수)
    - varchar2 substr(컬럼, 시작위치)
    - *** SQL은 one-based index 사용. 첨자를 1부터 시작~┌(。Д。)┐
*/
...
select 
    ...
    instr('안녕하세요. 홍길동님', '아무개') as c3, -- 찾는글자가 없으면 0반환
    instr('안녕하세요. 홍길동님. 반갑습니다. 홍길동님.', '홍길동') as c4,
    instr('안녕하세요. 홍길동님. 반갑습니다. 홍길동님.', '홍길동') as c5,
    instr('안녕하세요. 홍길동님. 반갑습니다. 홍길동님.', '홍길동', 11) as c6,
    instr('안녕하세요. 홍길동님. 반갑습니다. 홍길동님.', '홍길동'
            , instr('안녕하세요. 홍길동님. 반갑습니다. 홍길동님.', '홍길동') + length('홍길동')) as c7
from dual;
...
/*
    decode()
    - 문자열 치환
    - varchar2 decode(컬럼, [찾을 문자열, 바꿀 문자열] x N)
    - 못찾으면 null 반환(***)
    - case의 간단한 버전
