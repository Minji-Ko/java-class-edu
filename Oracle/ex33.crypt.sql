...
select id, pw, pkgCrypto.decrypt(pw, 'java') from tblUser where id <> 'hong';
select id, pw, pkgCrypto.decrypt(pw, 'oracle') from tblUser where id <> 'hong';

select pkgCrypto.decrypt('1234', 'java') from dual;


-- 단방향 암호화
-- dbms_crypto.hash
-- 터미널 열기 > sqlplus > sys as sysdba >  grant execute on dbms_crypto to public;
