...
select count(population) from tblCountry; -- 13 > null포함
select population from tblCountry;

select count(*) from tblCountry; --일부 컬럼값이 null이어도 다른 컬럼값이 있기때문에 모든 레코드 수 반환
...
--tblInsa. 총 직원수? 연락처 있는 직원수? 없는 직원수?
select 
    count(*) as "총 직원수",
...
select 
    ...
    sum(population) / count(population) -- count = 13 > null미포함
from tblCountry;
...
-- select * from tblInsa where basicpay >= (select avg(basicpay) from tblInsa); > 서브쿼리
