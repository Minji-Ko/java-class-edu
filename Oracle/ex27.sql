...
--insert into tblDate values (15, '결석','2022-03-15');
...
select v.regdate, 
    case
        when to_char(v.regdate, 'd') = '1' then '일'
        when to_char(v.regdate, 'd') = '7' then '토'
        else '*' || t.state
    end as state
from vwDate v
    left outer join tblDate t
...
insert into tblScore values (5,60,3,4);
            ...
            inner join tblSubject sb
                on sb.seq = r.subject_seq
                    inner join tblScore sc
                        ...
                        and r.subject_seq = sc.subject_seq;   -- (r.student_seq, r.subject_seq) = (sc.student_seq, sc.subject_seq) 아님!!!*****
