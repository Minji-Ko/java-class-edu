...
/*
    SELECT 컬럼리스트 > 컬럼을 선별하는 역할 > 컬럼 리스트에서 할 수 있는 행동
    FROM 테이블명 > 테이블 지정
    WHERE  ... ? TODO
...
-- 중복 값이 존재하지 않을 때 distinct > 아무일도 안일어난다.
select distinct name from tblCountry;

-- 중복값 없이 대륙을 가져오고, 각 대륙에 어떤 나라가 있는지 국가명도 함께 가져오시오.
select distinct continent from tblCountry;
-- *** 공용 데이터와 개별 데이터는 하나의 테이블에 넣을 수 없다.
-- *** 공용 데이터와 개별 데이터는 메모리 한 곳에 넣을 수 없다. > 공용(static), 개별(stack)
-- 셀 병합은 존재하지 않음
...
select gender, weight from tblComedian;
...
select
    ...
    case
        ...
        -- else '기타'
        -- else continent
        -- else name > 사용금지!!! > 하나의 속성 안에는 같은 도메인(성격, 표현)을 가지는 값이 들어간다.(****) 
        -- else population > ORA-00932: inconsistent datatypes: expected CHAR got NUMBER
...
select 
    ...
    case
        ...
        when weight > 50 then '정상체중'  -- 50 < weight <= 100
        else '저체중'
    end as state,
    case
...
-- 직위별 성과급 차등 지급 > 부장(x2), 과장(x1.7), 대리(x1.5), 사원(x1.2)
select 
    name, jikwi, sudang,
    case
...
-- 직위별 성과급 차등 지급 > 부장 & 과장(x2), 대리 & 사원(x1.5)
select 
    name, jikwi, sudang,
    case
