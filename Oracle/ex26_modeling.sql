...
/*
    ...
    - 개념 + 논리 + 물리
    - 개념 > 아주 간단하게 표현한 설계도 > 테이블 이름과 관계 정도만 기술 > (ex) 출결 <- 학생 -> 시험 -> 교과목 <- 교사  
    - 논리 > 관계형 데이터 베이스의 기본 설정 > 속성 정의 + 도메인 정의 + 키(PK,FK)... > 제대로된 ERD
    ...
    - 엔티티간의 관계를 표현한 그림
    - 데이터베이스 모델링 기법 중 하나
    - 손, 오피스, 전문툴(eX ERD, ER-WIN)
    
    2. Entity, 엔티티
    - 다른 Entity와 분류(구분)될 수 있고, 다른 Entitiy에 대해 정해진 관계를 맺을 수 있는 데이터 단위
    - 테이블 = 릴레이션 = 객체 = 레코드 = 엔티티
        a. 학생 정보 관리
    ...
    정규화, Normalization
    - 자료의 손실이나, 불필요한 정보를 없애고, 데이터의 일관성을 유지하며, 데이터 종속성을 최소화하기 위해 ERD을 수정하는 작업
    - 우리가 만든 테이블(ERD) > 비정형, 비정규화 상태 > 정규화 > 정규화된 구조
    - 제 1정규화 > 제 2정규화 > 제 3정규화 등..
    - 제 1~3정규화
    
    관계형 데이터베이스 시스템이 지향하는 데이터베이스의 상태
    1. 최대한 null상태의 값을 가지지 않는다.
    2. 중복값을 저장하지 않는다.
    3. 원자값(단일값)을 가진다.
    ...
    1. null 최대한 제거
    2. 중복값 제거
    3. 자료 삽입(insert), 갱신(update), 삭제(delete)에 따른 이상 현상 제거 > 데이터 무결성 보장
    ...
    2. 갱신 이상, Update Anomaly
        - 동일한 데이터가 2개이상의 테이블에 동시 저장 > 그 중 1개는 수정했지만 다른 1개를 수정하지 못했을 상황 
            > 두 데이터 간의 이상 오류 발생
    ...
    함수 종속, Functional Denpendency
    - 하나의 테이블내 컬럼끼리의 관계 표현
    - 정규화는 '부분 함수 종속'이나 '이행 함수 종속'을 모두 없애고, 모든 컬럼의 관계를 '완전 함수 종속'으로 만드는 작업이다.
    1. 완전 함수 종속, Full Functional Depencdency
    2. 부분 함수 종속, Partial Functional Depencdency
    3. 이행 함수 종속, Transitive Functional Depencdency
    ...
    - 주로 복합키를 가지는 테이블에서 발견되며, 
        일부 컬럼이 복합키 모두에게 종속이 아니라 복합키 일부에만 종속되는 현상이 발견 > 현상 제거
        
    제 3 정규화, 3NF
    - 기본 키가 아닌 다른 컬럼에서 종속되는 컬럼이 존재하면 안된다.
