...
public class Ex09_Casting {
	...
	public static void main(String[] args) {
		...
		/*
		 	...
		 	2. 명시적 형변환(= 강제 타입 변환)
		 		- 작은형 = 큰형
		 		- 경우에 따라 다름 > 작은형이 소화할 수 있는 범위의 원본값이면 복사가 가능하고, 그렇지 않은 범위의 원본값이면 손실분이 생김 > 성공/실패
		 		- 단, 개발자의 주의를 요함!!
		 	...
		 	- 변수 = 값(리터럴, 변수)
		 	- LValue = RValue
		 	- ***** LVale의 자료형과 RValue의 자료형은 반드시(******) 동일해야 한다. > 이 규칙을 만족하지 못하면 에러 발생!!(컴파일 에러)
		...
		//short = byte
		//2byte = 1byte
		//암시적인 형변환 발생 > 개발자 모르게 자료형을 바꾸는 행동이 발생!!!
 		s1 = b1;
 		
 		//(자료형)피연산자: 형변환 연산자 > 피연산자의 자료형을 ()안의 자료형으로 바꿔서 반환
 		//s1 = (short)b1;
 		...
 		//작은형 = 큰형
 		// 1칸 = 2칸
 		b2 = (byte)s2; 		//개발자에게 경고!!! 야!! 여기 잘 봐!! 틀릴 수 있으니까...
 		System.out.println(b2); //본사본
...
// 		- char <-> 정수
 		...
 		//결론: char를 숫자로 형변환하려면 반드시 int이상의 자료형으로 변환해야 한다.(65335까지를 표현할 수 있는 자료형이 필요)
 		char c3;
 		int a3;
