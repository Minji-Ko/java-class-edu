...
//		요구사항] 지도 > 우리집 > 좌표 > 저장 > 호출 + 출력
		...
		//Case1.
		//- 간편, 즉시 생성
		//- 식별자 문제	> 같은 성질의 식별자가 서로 다른 이름을 사용 > x, x2..
		//			> 물리적인 제약이 존재하지 않음 > 사용자 경험에 근거 > 구조 불안정 > x,y가 한짝?
		//- 생산성, 유지보수성 낮음
		...
		//- 동일 성질 데이터 > 동일한 식별자 사용(방번호)
		//- 요소간의 성질을 구분하기 힘들다. > [0], [1]이 어떤 데이터를 저장했는지 알 수 없음 > *** 서로 다른 성질의 데이터를 ㅓㄶ으면 안된다.
		
		//1명의 점수(5과목)
		int[] score = { 100, 90, 80, 70, 60 }; //서로 다른 성질의 데이터 > 비추천 ***
		...
		//Case3.
		//- 클래스 사용
		//- 클래스를 사용하여 정의한 내용을 메모리에 실체화시키는 행동
		//- 객체 생성하기(******)
		//- 클래스는 자료형의 역할을 한다. > 변수를 만들 수 있다.
		
		//- 데이터 저장의 구조 생성 > 관리 수월(용이) > 가독성 향상  
		//- 각 요소의 이름 부여 > 각각의 데이터가 어떤 데이터인지 식별이 용이 > 배열의 단점 완벽 커버!
		//- 요소의 이름이 동일히다. 
		...
		//2. 조합 부품(블럭 조합) > 새로운 모양 > 참조형(문자열, 배열, 클래스) x 무한대
