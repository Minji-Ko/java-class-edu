...
//		Cup p5;
		...
		clist[1] = new Cup("", 0); //2번째 컵
		clist[2] = new Cup("", 0); //3번째 컵
		clist[3] = new Cup("", 0); //4번째 컵
	...
	private static void m2() {
		...
		//- 순수 배열은 요소 삭제 불가능
		//- 컬렉션은 요소 삭제 가능
		//- T remove(int index) > 방번호를 찾아서 삭제 > 정확한 컨트롤 > 더 많이 사용함
		//- boolean remove(T value) > 값을 찾아서 삭제 > 처음 만나는 요소만 삭제 > 조심해야함!!!
		//- ***** 좌측 시프트가 발생한다!!! > 삭제된 방 이후의 모든 요소는 모두 방번호 -1 감소
		list.remove(2);
		list.remove("파인애플");
		...
		//for (변수 : 배열(컬렉션)) 
		for (String item : list) { //행동에 변화를 줄 수 없음 > but, 전체를 보는 일이 많아서 많이 쓰임
		...
		//- ***** 우측 시프트가 발생한다. > 삽입된 방 이후의 모든 요소가 방번호 + 1
		list.add(2, "망고");
	...
	private static void m1() {
		...
		System.out.println(num1[0]); //식별자[첨자] -> 인덱서(Indexer) > 자바는 배열만 지원
