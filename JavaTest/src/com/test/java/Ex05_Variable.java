...
//		정수형 리터럴은 int 범위를 벗어나 표현할 수 없다.
		//l1 = 2_300_000_000;    //The literal 2300000000 of type int is out of range 
		//System.out.println(l1);
	
		l1 = Long.MAX_VALUE;
		System.out.println(l1); //9223372036854775807
		
		l1 = 2_300_000_000L;    //Long형 리터럴 > 접미어(L)
		System.out.println(l1);
		
		l1 = 10L;				//이 경우 자유!!(int 범위 내에서는 접미어 자유롭게)
		System.out.println(l1);
...
//		변수만 자료형이 있는게 아니라, 모든 리터럴(상수)도 자료형이 있다.
		...
		System.out.println(10); //10이라는 숫자는 어떤 자료형? > int
...
//		int -> 64bit : 정수형 integer을 32bit로 고정	
//		int -> 32bit : (90년대말)혼란
//		int -> 16bit
...
//		(byte, short는 예전 운영체제를 위한 legacy)
...
//		보통 실수형은 잘 사용하지 않는다. > 손실분 발생때문에 반드시 필요한 경우나 손실분이 미미한 경우에만 사용한다.
...
//		- 참조형에 속함
//		- 문자의 열
//		- 문자들이 열을 지어 있는 형태 > 문자의 집합 > char이 여러개 모여있는 형태
...
//		보통 char보다 String을 많이 사용한다. > char만의 특징도 있음
